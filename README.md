...
- デストラクタ (`~ModemHandler`) を追加し、読み取りタスクとキューを破棄時に解放するようにしました。
- `begin()` を複数回呼び出しても読み取りタスクが増殖しないようにしました。
- レスポンスキュー / 非同期イベントキューが満杯のときに、取りこぼした行が解放されずヒープリークになる問題を修正しました。
- `sendATCommandWithResponse` と `getResponses` で、複数行応答の受信中に指定した合計タイムアウトを超えて待ち続ける可能性があった問題を修正しました（残り時間を渡すように変更）。
